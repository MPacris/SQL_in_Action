...
-- EXPECTED OUTPUT: Williamsburg  NOTE Williamsburg is first on list but there are 2** Williamsburg and Harlem
-- INVESTIGATE: Should Williamsburg be crowned the most popular neighbourhood?  
SELECT neighbourhood, COUNT(*) FROM intro_sql.final_airbnb group by neighbourhood order by count(*) DESC LIMIT 2;
